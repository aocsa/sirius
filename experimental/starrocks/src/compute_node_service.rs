...
impl SiriusComputeNodeService {
    ...
    fn try_dispatch_sender(
        ...
        if let Some(ready) = self.core.try_defer_sender(&params)? {
            self.dispatch_then_join(FragmentOutcome::from_ready(ready))?;
            return Ok(true);
        }
...
impl ServiceCore {
    ...
    fn try_defer_sender(
        ...
        // A leaf of a query this CN already failed is never deferred. Gate 4 in
        // `process_fragment` cannot cover this: on the async path `try_dispatch_sender` runs
        // this hook first. Declining sends the leaf down today's path, where gate 3 in
        // `run_ready_fragment` skips it without translating.
        if let Some(query_id) = query_id
            && let Some(cause) = self.results.failure_of(query_id)
        {
            debug!(
