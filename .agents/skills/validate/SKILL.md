...
**Reference:** See `.Codex/skills/_shared/build-and-query.md` for shared infrastructure (build modes, query execution, result comparison, autonomy mode, change tracking, debug log conventions).
...
2. **Establish baseline:**
   ...
   ```bash
   build/release/duckdb <db_path> -c "SELECT ..." > /tmp/Codex-1000/baseline_result.txt
   ```
...
3. **Run via Sirius GPU:**
   ...
   build/release/duckdb <db_path> -c "CALL gpu_execution('SELECT ...');" > /tmp/Codex-1000/gpu_result.txt
   ```
...
2. **Check with nsys:** Profile the query with `nsys` and examine the stream IDs:
   ```bash
   nsys profile --stats=true -o /tmp/Codex-1000/validate_profile build/<preset>/duckdb <db_path> <<'EOF'
   CALL gpu_execution('<QUERY>');
   EOF
   ```
